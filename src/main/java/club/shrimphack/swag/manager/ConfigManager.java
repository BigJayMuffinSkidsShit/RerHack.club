...
public class ConfigManager implements Util {
    ...
    public String config = "rerhack/config/";
    ...
    public void loadConfig(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("rerhack").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        if (files.contains(new File("rerhack/" + name + "/"))) {
            this.config = "rerhack/" + name + "/";
        } else {
            this.config = "rerhack/config/";
        }
    ...
    public boolean configExists(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("rerhack").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        return files.contains(new File("rerhack/" + name + "/"));
    }
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("rerhack/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("rerhack", ""));
                writer.close();
            } else {
